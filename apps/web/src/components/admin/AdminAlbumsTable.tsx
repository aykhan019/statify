...
import type { AdminAlbumListItem } from '@statify/shared';
import { EmptyState } from '@/components/states';
import { Button } from '@/components/ui/Button';
import { Icon } from '@/components/ui/Icon';
import { ApiClientError } from '@/lib/api-client';
import { setAdminAlbumHidden, updateAdminAlbum } from '@/lib/admin/api';
import { CopyButton } from './CopyButton';

interface AdminAlbumsTableProps {
  initial: AdminAlbumListItem[];
}
...
export function AdminAlbumsTable({ initial }: AdminAlbumsTableProps) {
  ...
  if (items.length === 0) {
    return <EmptyState title="No albums match your search" />;
  }
  ...
  async function applyEdit(id: number) {
    ...
    setError(null);
    try {
      const updated = await updateAdminAlbum(id, {
  ...
  async function applyHidden(id: number, hidden: boolean) {
    ...
    setError(null);
    try {
      const updated = await setAdminAlbumHidden(id, { hidden });
      setItems((prev) => prev.map((item) => (item.id === id ? updated : item)));
      startTransition(() => router.refresh());
    } catch (err) {
      setError(getErrorMessage(err));
    ...
    <div className="flex flex-col gap-3">
      ...
      <div className="overflow-x-auto">
        <table className="w-full text-sm">
          <caption className="sr-only">Admin albums</caption>
            ...
            {items.map((item) => {
              ...
              return (
                ...
                >
                  ...
                  <td className="px-3 py-3">{item.primaryArtistName}</td>
