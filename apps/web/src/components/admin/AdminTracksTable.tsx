...
import type { AdminTrackListItem } from '@statify/shared';
import { EmptyState } from '@/components/states';
import { Button } from '@/components/ui/Button';
import { Icon } from '@/components/ui/Icon';
import { ApiClientError } from '@/lib/api-client';
import { setAdminTrackHidden, updateAdminTrack } from '@/lib/admin/api';
import { CopyButton } from './CopyButton';

interface AdminTracksTableProps {
  initial: AdminTrackListItem[];
}
...
export function AdminTracksTable({ initial }: AdminTracksTableProps) {
  ...
  if (items.length === 0) {
    return <EmptyState title="No tracks match your search" />;
  }
  ...
  async function applyEdit(id: number) {
    ...
    setError(null);
    try {
      const updated = await updateAdminTrack(id, {
  ...
  async function applyHidden(id: number, hidden: boolean) {
    ...
    setError(null);
    try {
      const updated = await setAdminTrackHidden(id, { hidden });
      setItems((prev) => prev.map((item) => (item.id === id ? updated : item)));
      startTransition(() => router.refresh());
    } catch (err) {
      setError(getErrorMessage(err));
    ...
    <div className="flex flex-col gap-3">
      ...
      <div className="overflow-x-auto">
        <table className="w-full text-sm">
          <caption className="sr-only">Admin tracks</caption>
              ...
              return (
                ...
                >
                  ...
                  <td className="px-3 py-3 text-xs text-muted-foreground">
                    {item.albumName} · {item.primaryArtistName}
                  </td>
                  ...
                  <td className="px-3 py-3 tabular-nums">{item.playCount}</td>
