...
- **Next concrete action:** start P6-M2 in the next session. Author `DESIGN.md` at repo root (full token spec from the Vivid Workshop direction: oklch color set with semantic naming layer over a multi-hue palette plus a data-viz palette of at least eight hues; type scale with self-hosted webfont families and weight axis; spacing / radius / shadow / motion scales; image aspect ratio scale with frame and overlay treatments and the null-fallback rule; icon size scale with locked stroke weight; do / do not section) and `docs/adr/0002-design-system-and-token-layer.md` (records the ADR-001 §3.8 / §3.20 deviations and the `image_url` schema decision).
- **Follow-ups:**
