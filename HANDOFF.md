...
- **Next concrete action:** start P6-M10 (section identity headers + section-hue propagation, `rahila`) once Aykhan green-lights it. Branch `feat/p6-m10-section-headers` off latest `dev` (currently `3ad5c16`). Build a section provider that resolves the active hue from the route and sets the `--color-section-*` aliases, a full-bleed section-hue block header on every top-level route, and propagation to the active nav indicator, row hover, cover frames, and the chart series index-0 swap. Spec: `CHECKLIST.md` P6-M10 row + `DESIGN.md` §1.3. Entry criteria satisfied (P6-M5 and P6-M6 merged). Verify (`pnpm format:check`, `pnpm lint`, `pnpm typecheck`, `pnpm test`, `pnpm build`) plus a local UI smoke. Per-task commits via `scripts/commit-as.sh rahila ...`; one PR into `dev`; rebase-merge with `gh pr merge <n> --rebase --delete-branch`.
- **Follow-ups:**
