...
- **Current milestone:** P6-M9 Empty / loading / error states pass. Wait for explicit green light before starting; the M9 row is attributed to `rahila` (state primitives Skeleton / EmptyState / ErrorState / NotFoundState wired through every `(app)/**` list and detail route, DESIGN.md states addendum, `/styleguide` states section). Entry criteria satisfied: P6-M7 and P6-M8 are both merged.
