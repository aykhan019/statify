...
- **Last shipped:** P6-M9 Empty / loading / error states pass (branch `feat/p6-m9-states-pass`, PR into `dev`). Adds state primitives under `apps/web/src/components/states/` (a token shimmer `Skeleton` plus `ListSkeleton` / `CardGridSkeleton` / `DetailSkeleton` / `ChartSkeleton`, `EmptyState`, `ErrorState`, `NotFoundState`, and a shared `StatePanel`); exports `buttonVariants` from `ui/Button` so a `Link` can adopt the secondary-button look. Wires a `loading.tsx` skeleton per list / detail / stats segment, `(app)/error.tsx` (client, `ErrorState` + `reset`), and `(app)/not-found.tsx` (`NotFoundState`); routes the shared `InfiniteScroll` and every primary route-level zero-length case through `EmptyState`, with nested sub-section empties kept as plain `--fg-muted` lines. Documents the vocabulary in `DESIGN.md` §10 (Do / Do not → §11, Token summary → §12) and a `/styleguide` §19 panel. No new dependency. Lint / typecheck / build pass; `/styleguide` renders the panels; interactive smoke (network throttle for skeletons, forced 404 / 500 while authenticated) is left for the reviewer.
...
- **Current milestone:** P6-M10 Section identity headers + section-hue propagation. Wait for explicit green light before starting; the M10 row is attributed to `rahila` (a section provider that resolves the active hue from the route and sets the `--color-section-*` aliases, a full-bleed block header on every top-level route, and propagation to the active nav indicator, row hover, cover frames, and the chart series index-0 swap). Entry criteria satisfied: P6-M5 and P6-M6 are both merged. Note: the M9 branch also carries one `aykhan` commit that inserts the P6-M10 milestone and renumbers the Phase 6 tail.
...
- **Deployment gates:**
  ...
  2. M5, M6, M7, and M8 surfaces need an authed end-to-end browser smoke against a Node 22 API with seeded listening history, at least one seeded public user playlist, and at least one admin account before the dev → main promotion (the M8 sweep adds form-submission success + validation paths to that list).
- **Next concrete action:** start P6-M9 on a `feat/p6-m9-states-pass` branch off latest `dev` once green-lit. Build state primitives at `apps/web/src/components/states/` (Skeleton, EmptyState, ErrorState, NotFoundState) using token-only styling, then wire every list and detail route under `apps/web/src/app/(app)/**` to a skeleton during suspense, an empty state for zero-length responses, an error state on fetch failure, and a not-found state when the entity is missing. Document copy and icon choices in DESIGN.md, extend `/styleguide` with a states section, then verify (`pnpm format:check`, `pnpm lint`, `pnpm typecheck`, `pnpm test`, `pnpm build`) and smoke each state locally (throttle for skeletons, force 404, force 500). Per-task commits via `scripts/commit-as.sh rahila ...`; one PR into `dev`; rebase-merge with `gh pr merge <n> --rebase --delete-branch`.
- **Follow-ups:**
