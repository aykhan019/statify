...
- **Last shipped:** P6-M10 Section identity headers + section-hue propagation (PR #38, branch `feat/p6-m10-section-headers`). Adds `apps/web/src/components/section/` with a route-aware section resolver, provider, block header, and content wrapper; moves runtime section aliases to provider-driven `data-section-hue` CSS with dark-mode tint handling; adds full-width section block headers across top-level route surfaces while keeping `/me/account` neutral; wires active navigation/catalog tabs, row hover, cover frames, and chart series index 0 to the active section hue; documents block behavior in DESIGN.md §1.3.1 and renders all eleven section headers in `/styleguide`. No new dependency. Verification passed: `pnpm format:check`, `pnpm lint`, `pnpm typecheck`, `pnpm test`, `pnpm build`, PR CI, and authenticated local HTTP smoke against `/me`, `/me/account`, `/catalog/tracks`, `/discover`, `/explore/hidden-gems`, `/me/history`, `/me/playlists`, `/community/playlists`, `/me/stats`, `/me/stats/top-artists`, `/me/stats/top-tracks`, `/me/stats/heatmap`, `/me/stats/trending`, `/admin`, and `/styleguide`.
...
- **Current milestone:** P6-M11 Analytics surfaces re-skin. Green light already given; the M11 row is attributed to `aykhan` (shared chart theme module under `apps/web/src/components/charts/`, all stats surfaces wired to CSS-variable chart tokens, documented tooltip / legend treatment, and heatmap scale from the data-viz palette). Entry criteria satisfied: P6-M3 and P6-M10 are both merged.
...
- **Next concrete action:** start P6-M11 (analytics surfaces re-skin, `aykhan`) on `feat/p6-m11-analytics-reskin` off latest `dev` (currently `f16bb46`). Build a shared chart theme module at `apps/web/src/components/charts/` that reads axis, grid, tooltip, legend, and series colors from CSS variables; wire all existing stats pages through it; document the data-viz addendum in DESIGN.md; verify (`pnpm format:check`, `pnpm lint`, `pnpm typecheck`, `pnpm test`, `pnpm build`) plus local chart smoke. Per-task commits via `scripts/commit-as.sh aykhan ...`; one PR into `dev`; rebase-merge with `gh pr merge <n> --rebase --delete-branch`.
- **Follow-ups:**
