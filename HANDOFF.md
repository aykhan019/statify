...
- **Last shipped:** P6-M11 Analytics surfaces re-skin (PR #39, branch `feat/p6-m11-analytics-reskin`). Adds `apps/web/src/components/charts/` with a shared chart container, legend, and token helpers; wires stats bar charts and the heatmap through CSS-variable chart tokens with section hue at series index 0; documents tooltip, legend, and heatmap treatment in DESIGN.md §1.6.1; fixes analytics numeric mapper coercion so Decimal-like values return as numbers for stats routes. No new dependency. Verification passed: `pnpm format:check`, `pnpm lint`, `pnpm typecheck`, `pnpm test`, `pnpm build`, PR CI, authenticated local HTTP smoke for `/me/stats/top-artists`, `/me/stats/top-tracks`, `/me/stats/heatmap`, `/me/stats/trending`, and `/styleguide`, plus local browser smoke confirming chart render/reflow.
...
- **Current milestone:** P6-M12 Motion pass. Green light already given; the M12 row is attributed to `rahila` (motion tokens across navigation, list mounts, modal / dialog open / close, hover / focus transitions, player state changes, and global reduced-motion handling). Entry criteria satisfied: P6-M7, P6-M8, P6-M9, P6-M10, and P6-M11 are merged.
...
- **Next concrete action:** start P6-M12 (motion pass, `rahila`) on `feat/p6-m12-motion-polish` off latest `dev` (currently `ec35ed2`). Apply named motion tokens across navigation, list mounts, modal / dialog open / close, hover / focus transitions, and player state changes; add global `prefers-reduced-motion` handling; render motion tokens and reduce-motion preview in `/styleguide`; verify (`pnpm format:check`, `pnpm lint`, `pnpm typecheck`, `pnpm test`, `pnpm build`) plus local motion smoke. Per-task commits via `scripts/commit-as.sh rahila ...`; one PR into `dev`; rebase-merge with `gh pr merge <n> --rebase --delete-branch`.
- **Follow-ups:**
